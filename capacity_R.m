...
for variable_n = 1 : length(variable_s)
    time_fre_resources = variable_s(variable_n, 1);
    for ii = 1 : Nite
        pos = zeros(K, 3);
        theta = zeros(K, P);
        phi = zeros(K, P);
    ...
    capacity(variable_n, 2:4) = capacity(variable_n, 2:4)  / time_fre_resources;
    if length(variable_s)==variable_n
        capacity_cdf(:, 2:4) = capacity_cdf(:, 2:4) / time_fre_resources;
    else
    end
end
capacity(:, 2:4) = capacity(:, 2:4) / Nite;
...
xlabel('Number of time-frequency resources','Fontname','Times')
