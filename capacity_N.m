...
close all;
naz_s = [7; 11; 16; 24; 32;];
nel_s = [36; 56; 76; 106; 151;];
variable_s = naz_s .* nel_s;
maz = 3;
mel = 4;
m_arr = maz * mel;
Rmin = 10;
...
for variable_n = 1 : length(variable_s)
    naz = naz_s(variable_n, 1);
    nel = nel_s(variable_n, 1);
    n_arr = variable_s(variable_n, 1);
    U = zeros(n_arr, n_arr);
    for nx = 1 : naz
        for ny = 1 : nel
...
h1 = subplot(1,2,1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
plot(naz_s, capacity(1:length(variable_s), 1), 'k--s','LineWidth',1,'MarkerSize',10)
hold on
plot(naz_s, capacity(1:length(variable_s), 2), 'k--*','LineWidth',1,'MarkerSize',10)
plot(naz_s, capacity(1:length(variable_s), 3), 'k--o','LineWidth',1,'MarkerSize',12)
plot(naz_s, capacity(1:length(variable_s), 4), 'k-^','LineWidth',1,'MarkerSize',10)
xlim([min(naz_s), max(naz_s)])
le = legend('Opportunistic','max-min','PF','Proposed', 'Location', 'northwest');
set(le,'Fontname','Times')
set(gca,'XTick',naz_s)
xlabel('Number of horizontal BS antennas','Fontname','Times')
...
for plot_nn = 1 : 10
    capacity_plot(plot_nn, :) = capacity_cdf(K*plot_nn*0.1, :);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
